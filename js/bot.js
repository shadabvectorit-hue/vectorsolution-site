...
(function () {
  ...
  var L = {
    en: {
      ...
      badPhone: "That doesn't look like a complete number — please write the full mobile number (e.g. 0300 1234567).",
    ...
    ur_rm: {
      ...
      badPhone: "Ye number poora nahi lag raha — mehrbani farma kar poora mobile number likhein (misal: 0300 1234567).",
      no: /^(nahi|nahin|no)$/i,
    ...
    ur: {
      ...
      badPhone: "یہ نمبر مکمل نہیں لگ رہا — براہِ کرم پورا موبائل نمبر لکھیں (مثلاً 0300 1234567)۔",
  ...
  var leadId = "b" + Date.now().toString(36) + Math.random().toString(36).slice(2, 8);
  var lead = { source: "bot", leadId: leadId, stage: "partial", lang: "", name: "", whatsapp: "", service: "", budget: "", message: "" };
  var step = 0;
  var started = false;
  ...
  function userSay(text) {
    ...
    // dir="auto" — never force RTL on what the visitor typed, or a phone number
    // written "0336 313 8686" is displayed back to them reversed.
    if (t && t.dir === "rtl") { m.dir = "auto"; m.classList.add("rtl"); }
    m.textContent = text;
    msgs.appendChild(m);
    msgs.scrollTop = msgs.scrollHeight;
  }
  ...
  /* Saved twice: once as soon as we have a name + number (so an abandoned chat
     is still a lead), then again complete. Same leadId, so the inbox shows one. */
  function saveLead(stage) {
    lead.stage = stage;
    try {
      var data = new FormData();
      Object.keys(lead).forEach(function (k) { data.append(k, lead[k]); });
      data.set("message", "[" + lead.lang + "] " + (lead.message || (stage === "partial" ? "(chat started — not finished)" : "(bot lead)")));
      fetch("contact-submit.php", { method: "POST", body: data, headers: { Accept: "application/json" } })
        .then(function (r) { if (!r.ok) console.warn("VectorIT: lead not stored (HTTP " + r.status + ")"); })
        .catch(function () { console.warn("VectorIT: lead could not be sent — WhatsApp handover still carries it"); });
  ...
  function handleText(text) {
    if (step === 1) {
      ...
      // A contact number we can't dial is the same as no lead at all.
      if ((text.replace(/\D/g, "") || "").length < 10) {
        say(t.badPhone);
        return;
      }
      lead.whatsapp = text;
      saveLead("partial");           // banked now, before they can wander off
      say(t.askNeed);
